...
class ReinforceAlgorithm(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        actions = th.tensor(self.rollout_buffer.actions, device=self.device)
        observations = th.tensor(self.rollout_buffer.observations, device=self.device)
        ...
        for t_step in reversed(range(self.rollout_buffer.buffer_size)):
            ...
            returns.appendleft(self.gamma*disc_return_t + self.rollout_buffer.rewards[t_step])

            # stop at the end of new episode
            episode_starts = self.rollout_buffer.episode_starts[t_step]
            if episode_starts and not (t_step==self.rollout_buffer.buffer_size-1):
...
if __name__ == '__main__':
    ...
    env = gym.make(env_id)
    # env = make_vec_env(env_id, 2)

    algo = ReinforceAlgorithm("MlpPolicy", env, n_steps=100, verbose=1).learn(total_timesteps=100000, log_interval=10)
